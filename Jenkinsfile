pipeline {
    ...
    stages {
        ...
        stage('Lint') {
            ...
            steps {
                sh 'cd cinemahall/cinemahall && golangci-lint run --enable-all --skip-dirs proto -D wsl lll' //--deadline 20m --enable-all; --disable-all -E errcheck
                sh 'cd ../movies && golangci-lint run --enable-all -D wsl lll' 
